...
<template>
  <Modal :show="show" @close="closeModal" maxWidth="max-w-4xl">
    <template #title>
    ...
    <div
      ...
      <cropper-canvas v-if="isModalReady" :key="cropperKey" background class="w-full h-full block fade-in">
        <cropper-image ref="cropperImage" :src="imageObjectUrl" alt="Picture" rotatable scalable></cropper-image>
        <cropper-shade hidden></cropper-shade>
        <cropper-handle action="select" plain></cropper-handle>
        <cropper-selection :aspect-ratio="aspectRatio" initial-coverage="1" movable resizable ref="cropperSelection">
    ...
    <template #footer>
      <div class="w-full flex flex-wrap gap-4 items-center justify-between">
        ...
        <div class="flex gap-2">
          ...
          <div class="relative">
            <button ref="dropdownBtnRef" @click="toggleRatioDropdown"
              class="px-4 py-2 rounded-lg bg-secondary text-text font-medium hover:brightness-95 transition-all flex items-center gap-2"
              title="Aspect Ratio">
            ...
            <Teleport to="body">
              <div v-if="isRatioDropdownOpen" class="fixed inset-0 z-[99998]" @click="closeRatioDropdown"></div>
              <ul v-if="isRatioDropdownOpen" :style="dropdownStyle"
                class="menu p-2 shadow-2xl bg-background rounded-xl w-44 border border-secondary shadow-black/50">
                <li><a @click="setAspectRatio(NaN)"
                    :class="{ 'bg-primary/20 text-primary font-bold': aspectRatio === undefined }">Bebas</a></li>
                <li><a @click="setAspectRatio(1)"
                    :class="{ 'bg-primary/20 text-primary font-bold': aspectRatio === 1 }">1:1</a></li>
                <li><a @click="setAspectRatio(4 / 3)"
                    :class="{ 'bg-primary/20 text-primary font-bold': aspectRatio === 4 / 3 }">4:3</a></li>
                <li><a @click="setAspectRatio(16 / 9)"
                    :class="{ 'bg-primary/20 text-primary font-bold': aspectRatio === 16 / 9 }">16:9</a></li>
              </ul>
            </Teleport>
          </div>
          <button @click="reset"
        ...
        <!-- Action Buttons -->
        <div class="flex gap-2">
          <button @click="closeModal" class="px-4 py-2 rounded-lg text-text hover:bg-secondary transition-colors"
