  ...
  <div
    ...
    <table class="w-full text-sm text-left text-text border-collapse" :class="isMobile ? 'block' : 'min-w-[600px]'">
      ...
      <TransitionGroup tag="tbody" name="list" class="relative"
        ...
        <tr v-else v-for="loc in allLocations" :key="loc.id" class="transition-colors group relative"
          ...
          <td class="font-mono font-semibold bg-background group-hover:bg-secondary/5 transition-colors"
          ...
          <td class="space-x-4 bg-background group-hover:bg-secondary/5 transition-colors"
            :class="isMobile ? 'flex justify-end items-center pt-4' : 'px-6 py-4 text-center sticky right-0 z-20 shadow-[-4px_0_8px_-4px_rgba(0,0,0,0.05)]'">
  ...
  <Modal :show="isModalOpen" @close="isModalOpen = false" :title="isEditing ? 'Edit Lokasi' : 'Tambah Lokasi Baru'">
    <form @submit.prevent="handleSave" class="p-6 space-y-4">
      ...
      <div>
        ...
        <BaseSelect v-model="selectedLocation.purpose" :options="purposeOptions" emit-value :searchable="false"
