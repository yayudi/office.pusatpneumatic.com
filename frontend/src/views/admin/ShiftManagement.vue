  ...
  <div class="p-6">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="shift in shifts" :key="shift.id"
        class="bg-background border border-secondary/20 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
        ...
        <div class="flex items-start justify-between mb-4">
          ...
          <div class="flex gap-2">
            ...
            <button @click="deleteShift(shift.id)"
              class="p-2 text-danger hover:bg-danger/5 rounded-lg transition-colors" title="Hapus Shift">
              <font-awesome-icon icon="fa-solid fa-trash" />
            </button>
          </div>
        </div>

        <div class="flex items-center gap-4 bg-secondary/5 p-3 rounded-lg border border-secondary/10">
