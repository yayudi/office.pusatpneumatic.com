...
<template>
  <div class="space-y-6 animate-fade-in">
    <!-- Header -->
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-secondary/20 pb-4">
      <div>
        <h2 class="text-2xl font-bold text-text">Manajemen Saluran Penjualan</h2>
        <p class="text-text/60 text-sm mt-1">Kelola referensi nama toko / sales untuk form upload picking list.</p>
      </div>
      <button 
        @click="openAddModal"
            ...
            <tr v-for="item in channels" :key="item.id" class="hover:bg-secondary/5 transition-colors">
              <td class="px-6 py-4">
                <span class="px-2 py-1 rounded-md text-xs font-bold" 
                  :class="item.platform === 'Shopee' ? 'bg-[#ee4d2d]/10 text-[#ee4d2d]' : (item.platform === 'Tokopedia' ? 'bg-[#00AA5B]/10 text-[#00AA5B]' : 'bg-secondary/20 text-text/70')">
                  {{ item.platform }}
                </span>
              </td>
              <td class="px-6 py-4 font-bold text-text">{{ item.name }}</td>
              <td class="px-6 py-4 text-text/60 truncate max-w-xs">{{ item.description || '-' }}</td>
              <td class="px-6 py-4 text-center">
                <span v-if="item.is_active" class="px-2 py-1 bg-success/10 text-success text-xs font-bold rounded-lg">Aktif</span>
                <span v-else class="px-2 py-1 bg-danger/10 text-danger text-xs font-bold rounded-lg">Nonaktif</span>
              </td>
              <td class="px-6 py-4 text-right">
                <div class="flex justify-end gap-2">
                  <button @click="openEditModal(item)" class="w-8 h-8 rounded bg-primary/10 text-primary hover:bg-primary/20 flex items-center justify-center transition-colors" title="Edit">
    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-background border border-secondary/20 rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in">
        ...
        <div class="p-5 space-y-4">
          ...
          <div>
            <label class="block text-xs font-bold text-text/70 mb-1">Nama Toko / Sales <span class="text-danger">*</span></label>
            <input v-model="formData.name" type="text" placeholder="Contoh: Toko Maju Jaya" class="w-full bg-background border border-secondary/30 rounded-lg px-3 py-2 text-sm text-text focus:border-primary/50 focus:ring-1 focus:ring-primary/50 outline-none" />
          </div>
          
          <div>
            <label class="block text-xs font-bold text-text/70 mb-1">Keterangan (Opsional)</label>
            <textarea v-model="formData.description" rows="2" placeholder="Contoh: Akun cabang Budi" class="w-full bg-background border border-secondary/30 rounded-lg px-3 py-2 text-sm text-text focus:border-primary/50 focus:ring-1 focus:ring-primary/50 outline-none resize-none"></textarea>
          </div>
          ...
          <div class="pt-2">
            <label class="flex items-center gap-3 cursor-pointer group">
              <div class="relative flex items-center">
                ...
                <div class="w-10 h-5 bg-secondary/30 rounded-full peer-checked:bg-success transition-colors duration-200"></div>
                <div class="absolute left-1 top-1 w-3 h-3 bg-secondary rounded-full transition-transform duration-200 peer-checked:translate-x-5 shadow-sm"></div>
              </div>
              <div>
                <span class="text-sm font-bold text-text group-hover:text-primary transition-colors">Saluran Aktif</span>
