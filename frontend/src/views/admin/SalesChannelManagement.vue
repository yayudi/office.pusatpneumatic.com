...
<template>
  <div class="space-y-6 animate-fade-in">
            ...
            <tr v-for="item in channels" :key="item.id" class="hover:bg-secondary/5 transition-colors">
              ...
              <td class="px-6 py-4 text-center">
                <span v-if="item.is_active"
                  class="px-2 py-1 bg-success/10 text-success text-xs font-bold rounded-lg">Aktif</span>
    ...
    <Modal :show="isModalOpen" @close="closeModal" maxWidth="max-w-md">
      <template #title>
        <div class="-mt-1">
          <h3 class="font-bold text-text">{{ modalMode === 'add' ? 'Tambah Saluran Penjualan' : 'Edit Saluran Penjualan'
            }}</h3>
        </div>
      </template>
      ...
      <div class="space-y-4">
        ...
        <div>
          <label class="block text-xs font-bold text-text/70 mb-1">Nama Toko / Sales <span
              class="text-danger">*</span></label>
          <input v-model="formData.name" type="text" placeholder="Contoh: Toko Maju Jaya"
            class="w-full bg-background border border-secondary/30 rounded-lg px-3 py-2 text-sm text-text focus:border-primary/50 focus:ring-1 focus:ring-primary/50 outline-none" />
        </div>

        <div>
          <label class="block text-xs font-bold text-text/70 mb-1">Keterangan (Opsional)</label>
          <textarea v-model="formData.description" rows="2" placeholder="Contoh: Akun cabang Budi"
            class="w-full bg-background border border-secondary/30 rounded-lg px-3 py-2 text-sm text-text focus:border-primary/50 focus:ring-1 focus:ring-primary/50 outline-none resize-none"></textarea>
        </div>
        ...
        <div class="pt-2">
          <label class="flex items-center gap-3 cursor-pointer group">
            <div class="relative flex items-center">
              ...
              <div class="w-10 h-5 bg-secondary/30 rounded-full peer-checked:bg-success transition-colors duration-200">
              </div>
              <div
      ...
      <template #footer>
        <div class="flex justify-end gap-3 w-full">
          <button @click="closeModal"
            class="px-4 py-2 text-sm font-bold text-text/60 hover:text-text transition-colors">Batal</button>
          <button @click="saveChannel" :disabled="isSaving"
