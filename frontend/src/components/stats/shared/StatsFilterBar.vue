  ...
  <div class="bg-background border border-secondary p-4 rounded-xl flex flex-col gap-4 shadow-sm mb-6">
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
      ...
      <div class="flex flex-wrap gap-2 w-full lg:w-auto align-bottom">
        <button v-if="showAdvancedToggle" @click="handleToggle"
          ...
          <font-awesome-icon :icon="showAdvanced ? 'fa-solid fa-chevron-up' : 'fa-solid fa-chevron-down'"
    ...
    <div v-show="showAdvanced" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 animate-fade-in">
      <slot name="advanced" />
    </div>
  </div>
</template>
