...
<template>
  <Modal :show="isOpen" @close="close" title="Batch Edit Produk" maxWidth="max-w-3xl">
    <div class="flex flex-col h-[600px] md:h-[550px]">
      ...
      <div class="flex-1 overflow-hidden flex flex-col">
        ...
        <div v-if="activeTab === 'export'" class="flex-1 flex flex-col h-full overflow-hidden">
          <p class="text-sm text-text/70 mb-4 shrink-0 font-medium">Download data terbaru untuk diedit. Pilih format Excel untuk kemudahan atau CSV untuk kompatibilitas.</p>
              ...
              <div v-for="job in exportJobs" :key="job.id" class="flex items-center justify-between p-3.5 border border-secondary/20 rounded-xl bg-background hover:border-primary/30 transition-colors shadow-sm group">
                <div>
                  ...
                  <p v-else class="text-xs text-text/80 font-bold flex items-center gap-1.5">
                    ...
                    Data Master Produk ({{ job.filters ? (JSON.parse(job.filters).format || 'xlsx').toUpperCase() : 'XLSX' }})
                  </p>
                </div>
        ...
        <div v-if="activeTab === 'import'" class="flex-1 flex flex-col h-full overflow-hidden">
          <p class="text-sm text-text/70 mb-4 shrink-0 font-medium">Upload file master produk yang sudah Anda edit. Sistem akan memproses pembaruan secara massal.</p>
          ...
          <label class="shrink-0 mb-6 bg-warning/5 border border-warning/20 rounded-2xl p-4 flex items-start gap-4 cursor-pointer hover:bg-warning/10 transition-colors">
            ...
            <div class="flex-1">
              ...
              <p class="text-xs text-text/70 mt-1.5 leading-relaxed font-medium">Sistem akan melakukan simulasi upload untuk mengecek validitas data dan menampilkan error jika ada, <strong>tanpa menyimpan perubahan apapun</strong> ke database.</p>
            </div>
          </label>
