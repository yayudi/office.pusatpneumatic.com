...
<template>
  <div class="flex flex-col gap-3">
      ...
      <button
        ...
        :class="
          isExpanded || activeFilterCount > 0
            ...
            : 'bg-background border-secondary text-text/60'
        "
        :title="isExpanded ? 'Tutup Filter' : 'Buka Filter'"
      >
    ...
    <!-- ================================================================ -->
    <button
      v-if="needsStandaloneToggle"
      @click="isExpanded = !isExpanded"
      class="flex items-center justify-between w-full px-3 py-2 rounded-lg border transition-all"
      :class="
        isExpanded || activeFilterCount > 0
          ? 'bg-primary/10 border-primary text-primary'
    ...
    <div
      ...
      :class="isExpanded ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden pointer-events-none'"
    >
