...
function handleSubmit() {
  ...
  // Logic FormData
  if (props.multiple) {
    files.value.forEach(f => formData.append(props.fileFieldName, f))
  } else {
  ...
  <div class="w-full space-y-4">
    ...
    >
      ...
      <div class="flex flex-col items-center justify-center gap-3 py-2">
        ...
        <div
          ...
          :class="
            files.length > 0 ? 'bg-success/10 text-success' : 'bg-primary/10 text-primary group-hover:bg-primary/20'
          "
        >
          <font-awesome-icon
            :icon="files.length > 0 ? 'fa-solid fa-file-circle-check' : 'fa-solid fa-cloud-arrow-up'"
            class="text-xl"
          />
        </div>
        ...
        <div class="space-y-1">
          ...
          <p class="text-[10px] text-text/40 uppercase font-bold tracking-wider">Format: {{ accept }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="pt-2">
      <label class="block text-[10px] font-bold uppercase text-text/40 mb-2 tracking-wider"
        >Catatan Tambahan <span class="text-text/40 font-normal">(opsional)</span></label
      >
      <textarea
        v-model="userNotes"
        rows="2"
    ...
    <div class="pt-2 space-y-4">
      ...
      >
        ...
        <span class="text-sm tracking-wide">{{
          loading ? (isDryRun ? 'Simulasi...' : 'Mengupload...') : isDryRun ? 'CEK VALIDASI' : submitLabel
        }}</span>
      </button>
    </div>
  </div>
